...
No Python or pip required. On [GitHub Releases](https://github.com/rcuevaspantoja/terminal-radio/releases), download the file named **`terminal-radio-*-windows-x64-portable.zip`**, unzip, and run **`radio.exe`**. Includes mpv.

Optional (future): **`TerminalRadio-setup.exe`** installer (Start Menu shortcut).
